#!/usr/bin/env bash
# shellcheck disable=SC2178,SC2206,SC2207
...
function delete_mods() {
    ...
    for target_mod_id in "${delete_mods_list[@]}"; do
        ...
        # 删除mod文件夹
        declare all_path
        all_path=$(echo "$all_mods_dir" | grep -s "/$target_mod_id")
    ...
    for cluster in "${array[@]}"; do
        ...
        if ! grep -sq "\[\"workshop-" "${mod_file_list[0]}"; then continue; fi  # mod配置文件里没有配置的话跳过
