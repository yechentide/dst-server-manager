...
function configure_mods_in_cluster() {
    ...
    declare -a mod_file_list
    mod_file_list=($(find "$KLEI_ROOT_DIR/$WORLDS_DIR_NAME/$answer" -type f -name modoverrides.lua))
    if [[ ${#mod_file_list} == 0 ]]; then color_print error '未找到modoverrides.lua!'; return 0; fi
    if [[ $1 == 'update' ]] && ! grep -sq "\[\"workshop-" "${mod_file_list[0]}"; then
